...
INSERT INTO CH10_DEPT(DEPTNO, DNAME, LOC) VALUES (70, 'SELECT', 'INCHEON');

SELECT * FROM CH10_DEPT;

UPDATE CH10_EMP SET DEPTNO = 70
WHERE SAL > (SELECT AVG(SAL) FROM EMP WHERE DEPTNO=50);

SELECT * FROM CH10_EMP;

UPDATE CH10_EMP SET SAL = SAL*1.1, DEPTNO = 80
    WHERE HIREDATE > (SELECT MIN(HIREDATE) FROM CH10_EMP WHERE DEPTNO = 60)

-- 5.
DELETE FROM CH10_EMP WHERE EMPNO = (SELECT EMPNO FROM EMP JOIN SALGRADE S ON EMP.SAL BETWEEN S.LOSAL AND S.HISAL WHERE S.GRADE = 5)
